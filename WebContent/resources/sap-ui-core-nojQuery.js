...
(function() {
	...
	if ( !oJQVersion.inRange("1.7.0", "2.0.0") ) {
		_earlyErrors.push("SAPUI5 requires a jQuery version of 1.7 or higher, but lower than 2.0; current version is " + jQuery.fn.jquery);
	}
	 ...
	 */
	var mModules = {
		// predefine jquery.sap.global to avoid redundant loading
		"jquery.sap.global" : { state : 'ready', url : _sBootstrapUrl }
	};
		...
		if ( mod && mod.state === 'loaded' && typeof mod.data !== "undefined" ) {
			try {
				...
				if ( typeof mod.data === "function" ) {
					...
					var oUri = URI(mod.url);
					var absoluteUrl = oUri.absoluteTo(sDocumentLocation);
					_window.eval(mod.data + "\r\n//@ sourceURL=" + absoluteUrl); // Firebug, Chrome and Safari debugging help, appending the string seems to cost ZERO performance
				}
				_execStack.pop();
				mod.state = 'ready';
				mod.data = undefined;
...
(function(){
   ...
   * @SecPassthrough {0|return}
	...
	jQuery.sap.formatMessage = function formatMessage(sPattern, aValues) {
		...
		jQuery.sap.assert(aValues == null || jQuery.isArray(aValues), "values must be an array or omitted");
		aValues = aValues || [];
...
(function() {
	...
	jQuery.sap.arrayDiff = function(aOld, aNew, fnCompare){
		...
		var fnFindSymbol = function(aSymbols, vData) {
			for (var i = 0; i < aSymbols.length; i++) {
				if (fnCompare(aSymbols[i].data, vData)) {
					return i;
				}
			}
			return null;
		...
		for (var i = 0; i < aNew.length; i++) {
			var iSymbolIndex = fnFindSymbol(oNewSymbols, aNew[i]);
			if (iSymbolIndex == null) {
				iSymbolIndex = oNewSymbols.length;
				oNewSymbols.push({ rows: [], o: null, data: aNew[i] });
			}
			oNewSymbols[iSymbolIndex].rows.push(i);
		}

		//Pass 2: collect symbols for old array
		for (var i = 0; i < aOld.length; i++) {
			var iSymbolIndex = fnFindSymbol(oOldSymbols, aOld[i]);
			if (iSymbolIndex == null) {
				iSymbolIndex = oOldSymbols.length;
				oOldSymbols.push({ rows: [], n: null, data: aOld[i] });
			}
			oOldSymbols[iSymbolIndex].rows.push(i);
		}

		//Pass 3: Find matches for unique lines
		for (var i = 0; i < oNewSymbols.length; i++) {
			var iOldSymbolIndex = fnFindSymbol(oOldSymbols, oNewSymbols[i].data);
			if (oNewSymbols[i].rows.length == 1 && iOldSymbolIndex != null && oOldSymbols[iOldSymbolIndex].rows.length == 1) {
				aNewRefs[oNewSymbols[i].rows[0]] = {
					data: aNew[oNewSymbols[i].rows[0]],
					row: oOldSymbols[iOldSymbolIndex].rows[0]
				};
				aOldRefs[oOldSymbols[iOldSymbolIndex].rows[0]] = {
					data: aOld[oOldSymbols[iOldSymbolIndex].rows[0]],
...
sap.ui = jQuery.extend(sap.ui, {
		 ...
		 */
		version: "1.15.0-SNAPSHOT",
		buildinfo : { lastchange : "${ldi.scm.revision}", buildtime : "20130809-0734" }
	});
...
(function() {
	...
	sap.ui.base.Object.extend("sap.ui.core.Configuration", /** @lends sap.ui.core.Configuration.prototype */ {
    	...
    	if (oResponse.success) {
			...
			var rCVer = /^(cversion\-+)([a-zA-Z0-9\-]+)$/,
				mCver, cVer;
			this._cversion = {};
			for (var n in oCfg) {
				mCver = rCVer.exec(n);
				if(mCver && mCver[2]){
					cVer = jQuery.sap.Version(oCfg[n]);
					//Only major and minor version are relevant
					cVer = jQuery.sap.Version(cVer.getMajor(), cVer.getMinor());
					this._cversion[mCver[2].toLowerCase()] = cVer;
				}
			}
		...
		getCompatibilityVersion : function (sFeature) {
			if(typeof(sFeature) === "string" && this._cversion[sFeature]){
				return this._cversion[sFeature];
			}
			
			var cVer = this._cversion._default;
			if(!cVer){
				this._cversion._default = cVer = jQuery.sap.Version(this.getVersion().getMajor(), this.getVersion().getMinor());
			}
			return cVer;
		},
 ...
 * @public
 */
sap.ui.model.Model.prototype.refresh = function() {
	this.checkUpdate();
};
 ...
 * @private
 */
sap.ui.model.Model.prototype.checkUpdate = function() {
...
(function(){
	 ...
	 *
	 * This class provides firing of resize events on all browsers by regularly 
	 * checking width and height of registered DOM elements and firing events accordingly.
	 *
	 * @class Monitors registered DOM elements for a resize and notifies listeners accordingly.
	...
	sap.ui.core.ResizeHandler.prototype.attachListener = function(oDomRef, fHandler){
		...
		log.debug("registered " + (oDomRef.id ? oDomRef.id : oDomRef) + ": " + sId);
		...
		jQuery.each(this.aResizeListeners, function(index, oResizeListener){
			if(oResizeListener){
				...
				if ( jQuery.contains(document.documentElement, oDomRef) && //check that domref is still active 
					...
					if ( bDebug ) { 
						log.debug("resize detected for '" + (oDomRef.id ? oDomRef.id : oDomRef) + "': " + oResizeListener.iWidth + "x" + oResizeListener.iHeight + " -> " + oDomRef.offsetWidth + "x" + oDomRef.offsetHeight); 
					}
...
}; // end of sap.ui.app.ApplicationMetadata
...
sap.ui.base.Object.defineClass("sap.ui.core.Core", {
		...
		publicMethods : ["boot", "isInitialized","isThemeApplied","attachInitEvent","attachInit","getRenderManager","createRenderManager",
						 ...
						 "isStaticAreaRef", "createComponent", "getApplication"]
	});
...
//	if ( oConfig.areas && oConfig.areas.length > 0 ) {
	...
	if (sApplication) {
		log.info("Loading Application",null,METHOD);
...
sap.ui.core.Core.prototype.createComponent = function(vComponent, sUrl, sId, mSettings) {
	var sName = vComponent;
	// Support object parameter instead of single parameters
	if (typeof vComponent == "object") {
		sName = vComponent.name;
		sUrl = vComponent.url;
		sId = vComponent.id;
		mSettings = vComponent.settings;
	}
