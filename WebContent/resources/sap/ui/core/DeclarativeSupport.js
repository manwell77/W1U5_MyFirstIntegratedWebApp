...
sap.ui.core.DeclarativeSupport._createControl = function(oElement, oView) {
	...
	// For DesignMode=true we need to reference controls (e.g. views) which are loaded lazy during runtime 
	var bLazy = $element.attr("data-custom-data:sap-dt-lazy") === "true";

	if (bLazy) {
		$element.remove();
		// When DesignMode=false, we need to skip controls which are flagged with "sapDtLazy" custom data
