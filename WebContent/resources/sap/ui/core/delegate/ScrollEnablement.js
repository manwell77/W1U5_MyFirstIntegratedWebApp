...
(function($) {
	...
	var oNativeMouseScrollDelegate = {
		...
		onAfterRendering: function() {
			...
			if (!bHasHeight || jQuery.browser.msie) {
				...
				this._sResizeListenerId = sap.ui.core.ResizeHandler.register($Container[0], function(){
					...
					if ($Container.height() > 0) {
						...
						if (!jQuery.browser.msie) { // for IE the resize listener must remain in place for the case when navigating away and coming back. For the other browsers it seems to work fine without.
