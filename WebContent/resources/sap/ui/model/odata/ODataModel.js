...
sap.ui.model.Model.extend("sap.ui.model.odata.ODataModel", /** @lends sap.ui.model.odata.ODataModel */ {
	...
	constructor : function(sServiceUrl, bJSON, sUser, sPassword, mHeaders, bTokenHandling, bWithCredentials, bLoadMetadataAsync) {
		...
		if (typeof bJSON === "object") {
			...
			this.sMaxDataServiceVersion = bJSON.maxDataServiceVersion;
			this.bUseBatch = bJSON.useBatch;
			bJSON = bJSON.json;
		}
	...
	function readRequestedData(oRequest){
		...
		if (that.bUseBatch) {
			...
			if (that.bTokenHandling && !that.bTokenRequested) {
				function _fnRefreshTokenSuccess(oData, oResponse) {
					if (oResponse) {
...
sap.ui.model.odata.ODataModel.prototype._submitChange = function(oRequest, fnSuccess, fnError){	
	...
	function _handleSuccess(oData, oResponse) {
		...
		// delete request from queue: if it is a create we also delete the data from oData
		jQuery.each(that.oRequestQueue,function(sName, oRequest){
			oRequest === oRequestToDelete ? delete that.oRequestQueue[sName] : false;
			delete that.oData[sName];
			delete that.mContexts["/" + sName]; // contexts are stored starting with /
		})
		
		if (that._isDataStored(oRequest) || oRequest.method == "POST"){
			that.refresh();
		}
...
sap.ui.model.odata.ODataModel.prototype._submitBatch = function(oRequest, fnSuccess, fnError){	
	...
	return OData.request(oRequest, _handleSuccess, _handleError, OData.batchHandler, undefined, this.getServiceMetadata());
 ...
 * @public
 */
sap.ui.model.odata.ODataModel.prototype.createEntry = function(sCollection, vProperties) {
	var oEntity = {};
	if (typeof vProperties === "object" && !jQuery.isArray(vProperties)) {
		oEntity = vProperties;
	} else {
		var oEntityMetadata = this.oMetadata._getEntityTypeByPath(sCollection);
		if (!oEntityMetadata) {
			jQuery.sap.assert(oEntityMetadata, "No Metadata for collection "+sCollection+" found");
			return undefined;
		}
	...
	var sKey = sCollection+"('"+jQuery.sap.uid()+"')";
	
	this.oData[sKey] = oEntity;
	
	oEntity.__metadata = {type: ""+this.oMetadata._getEntityTypeName(sCollection)};
	
	var sChangeKey = this._getChangeUrl('/'+sCollection); 
	
	var oRequest = this._createChangeRequest(sChangeKey, oEntity, "POST", true);
	
	this.oRequestQueue[sKey] = oRequest;
	
	return this.getContext("/"+sKey);
};
