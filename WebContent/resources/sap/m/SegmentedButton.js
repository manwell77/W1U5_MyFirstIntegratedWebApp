...
sap.m.SegmentedButton.prototype.onAfterRendering = function() {
	...
	var bAllIcons = true;
	var bImageUsed = false;
	var self = this;
	for(var i=0; i < aButtons.length; i++) {
		if(aButtons[i].getIcon() == "") {
			bAllIcons = false;
		}
		if(aButtons[i]._image && aButtons[i]._image.getMetadata().getName() == "sap.m.Image") {
			bImageUsed = true;
		}
	}
	if(bAllIcons) {
...
sap.m.SegmentedButton.prototype._fCalcBtnWidth = function() {
	...
	for(var i = 0; i < iItm; i++) {
		if (!isNaN(iMaxWidth)) {
			//+1px is needed for chrome / +2px for IE9(10)
			$this.children('#' + this.getButtons()[i].getId()).width(iMaxWidth+2);
		}
	}
};
/**
