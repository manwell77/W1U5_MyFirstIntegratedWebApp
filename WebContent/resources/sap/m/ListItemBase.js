...
sap.m.ListItemBase.prototype.init = function(){
	...
	this._singleSelectMasterAsActive = sap.ui.core.theming.Parameters.get("sapUiListSingleSelectMasterAsActive") == "true";
};
	...
	if (this._includeItemInSelection && (this._mode === sap.m.ListMode.SingleSelect || this._mode === sap.m.ListMode.SingleSelectLeft || this._mode === sap.m.ListMode.MultiSelect) || this._mode == sap.m.ListMode.SingleSelectMaster) {
		...
		case sap.m.ListType.Navigation:
			...
			if (!this._eventHandledByControl) {
				//timeout is the only way to get the ui renderer working...otherwise no active feedback
				setTimeout(function(){
					that.fireTap({/* no parameters */}); // (This event is deprecated, use the press event instead)
					that.firePress({/* no parameters */});
				},50);
			}
			if (activationHandled && (!this._eventHandledByControl)) {
...
sap.m.ListItemBase.prototype.ontouchstart = function(oEvent) {
	...
	this._timeoutIdStart = window.setTimeout(function() {
		...
		if (!(that._mode == sap.m.ListMode.SingleSelectMaster && that._singleSelectMasterAsActive) && !(that._includeItemInSelection && (that._mode === sap.m.ListMode.SingleSelect || that._mode === sap.m.ListMode.SingleSelectLeft || that._mode === sap.m.ListMode.MultiSelect)) && ((_event.touches && _event.touches.length === 1) || !_event.touches)) {
...
sap.m.ListItemBase.prototype._ontouchmove = function(oEvent) {
	...
	if ((this._active || this._timeoutIdStart) && touchMovement && !(this._mode == sap.m.ListMode.SingleSelectMaster && this._singleSelectMasterAsActive)) {
...
sap.m.ListItemBase.prototype._ontouchend = function(oEvent) {
	...
	// several fingers could be used
	if (((oEvent.targetTouches && oEvent.targetTouches.length === 0) || !oEvent.targetTouches) && !(this._mode == sap.m.ListMode.SingleSelectMaster && this._singleSelectMasterAsActive)) {
		var type = this.getType();
		var that = this;
		switch (type) {
		case sap.m.ListType.Active:
