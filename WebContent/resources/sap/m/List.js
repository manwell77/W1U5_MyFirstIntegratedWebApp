...
sap.m.List.prototype.onAfterRendering = function() {
	...
	if (aColumns.length) {
		var $table = jQuery.sap.byId(this.getId() + "-listUl");
		aColumns.forEach(function(oColumn) {
			oColumn.onColumnRendered($table);
		});
	}
};

sap.m.List.prototype.setBackgroundDesign = function(sBgDesign) {
	var sBgDesignOld = this.getBackgroundDesign();

	this.setProperty("backgroundDesign", sBgDesign, true);
	jQuery.sap.byId(this.getId()).removeClass("sapMListBG" + sBgDesignOld).addClass("sapMListBG" + this.getBackgroundDesign());
	return this;
};

sap.m.List.prototype.setShowSeparators = function(sSeparators) {
...
sap.m.List.prototype._setSelectedItem = function(oListItem, select) {
	if (select !== undefined) { // no select value given
		switch (this.getMode()) {
		...
		jQuery.sap.log.warning(oListItem + ": Cannot set the selection property for this list item, since no value is given.");
	}
};
...
sap.m.List.prototype.onswipeleft = function(oEvent) {
	if (this.getSwipeDirection() != "LeftToRight") {
...
sap.m.List.prototype.onswiperight = function(oEvent) {
	if (this.getSwipeDirection() != "RightToLeft") {
		this._onSwipe(oEvent);
	}
};
