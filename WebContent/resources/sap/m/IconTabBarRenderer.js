 ...
 * @private
 */
sap.m.IconTabBarRenderer._aAllIconColors = ['sapMITFilterCritical', 'sapMITFilterPositive', 'sapMITFilterNegative', 'sapMITFilterDefault'];
...
sap.m.IconTabBarRenderer.render = function(oRM, oControl){
	...
	jQuery.each(aItems, function(iIndex, oItem) {
		...
		if (oItem instanceof sap.m.IconTabFilter) {
			...
			oRM.write(">");
			if (!oItem.getShowAll()) {
				oRM.renderControl(oItem._getImageControl(['sapMITFilterIcon', 'sapMITFilter' + oItem.getIconColor()], oControl, sap.m.IconTabBarRenderer._aAllIconColors));	
			}
